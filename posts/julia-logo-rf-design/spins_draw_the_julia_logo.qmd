...
![Fig. 2: The scanner selects a thin slice by briefly applying a magnetic-field gradient and a shaped radio‑frequency (RF) pulse. The RF pulse only tips spins inside the slice (shown in yellow); spins outside remain aligned with the main field and are not detected.](figures/rf_excitation.gif)
...
![Fig. 4: A "pencil‑beam" excitation. A spiral gradient steers which location is in resonance while a jinc‑shaped RF pulse tips spins at those locations, producing a narrow cylindrical region of excited signal (like a pencil).](figures/rf_2dexcitation.gif)
...
![Fig. 5: Respiratory trace from a pencil‑beam excitation. The beam monitors motion along the head–foot (z) axis, the blue band is the acceptance window around end‑expiration. [Source](https://i-mri.org/ViewImage.php?Type=F&aid=727808&id=F7&afn=1040_IMRI_25_4_209&fn=imri-25-209-g007_1040IMRI)](figures/cardiac_mri_2d_excitation.png)
...
```julia
calc_grad!(∇loss, x, params) = FiniteDiff.finite_difference_gradient!(∇loss, x -> loss(x, params), x)
```
